...
export async function GET() {
    try {
        ...
        const servers: any[] = await query(
            `SELECT id, name, ip_address, domain, protocol, status, is_active,
        ...
        const resellers: any[] = await query(
            'SELECT id, reseller_id, max_users, allocated_traffic_gb FROM reseller_limits'
        );
        ...
        return new NextResponse(JSON.stringify(backup, null, 2), {
            headers: {
                ...
                'Content-Disposition': 'attachment; filename="powervpn-backup.json"',
            },
        });
    } catch (e: any) {
